...
io.on("connection", (socket) => {
  ...
  socket.on("task-created", async (task) => {
    try {
      ...
      for (const userId of assignedUsers) {
        ...
        if (userSocket) {
          userSocket.emit("notifications", notification);
        }
      }
  ...
  socket.on("subtask-updated", async (subtask) => {
    try {
      ...
      for (const userId of assignedUsers) {
        const notification = await createNotification({
          type: "subtask",
          message: `A subtask "${subtask.title}" was updated.`,
          recipient: userId,
          subtaskId: subtask._id,
          taskId: subtask.taskId, // Link to the parent task
        });
  ...
  socket.on("comment-created", async ({comment, usersId}) => {
    try {
      const mentionedUsers = usersId; // Array of user IDs

      // Fetch the admin user ID from the database (for example, if you have an admin role or a specific admin)
      const admin = await User.findOne({ role: "admin" }); // Adjust query to match your database schema
      const adminUserId = admin ? admin._id : null; // Use the admin's user ID if found, or null if not

      // Send notifications to mentioned users
      for (const userId of mentionedUsers) {
        const notification = await createNotification({
          type: "comment",
          message: `A new comment was created: "${comment.text}".`,
          recipient: userId,
      ...
      // If an admin exists, send them a notification about the new comment (if adminUserId is valid)
      if (adminUserId) {
        const adminNotification = await createNotification({
          type: "comment",
          message: `A new comment was created: "${comment.text}".`,
          recipient: adminUserId, // Admin user receives this notification
          commentId: comment._id,
          taskId: comment.taskId,
          subtaskId: comment.subtaskId,
        });

        // Emit to the admin if online
        const adminSocket = findSocketByUserId(adminUserId);
        if (adminSocket) {
          adminSocket.emit("notifications", adminNotification);
        }
      }
  ...
  try {
    ...
    socket.emit("notifications", notifications); // Send notifications back
    console.log(`Sent notifications to user ${userId}`); // Debugging log
  } catch (error) {
    console.error("Error fetching notifications:", error);
    socket.emit("error", { message: "Failed to fetch notifications" });
  }
